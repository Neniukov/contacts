...
class ProgressFragmentDialog: DialogFragment() {
    ...
    companion object {
        ...
        private fun showWhenNotOnScreen(dialog: ProgressFragmentDialog, fragmentManager: FragmentManager) {
            val dialogInStack = (fragmentManager.findFragmentByTag(ProgressFragmentDialog::class.java.name) as? ProgressFragmentDialog)
            if(dialogInStack == null) dialog.show(fragmentManager, ProgressFragmentDialog::class.java.name)
        }
